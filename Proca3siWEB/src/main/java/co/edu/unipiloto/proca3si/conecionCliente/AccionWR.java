...
public class AccionWR {
	...
	private static final String PATH_PROVEEDOR = "accion"; 
	...
	public AccionDTO crearAccion(AccionDTO accionDTO) throws Exception {
		Client client = ClientBuilder.newClient();
		System.out.println("Ejecutando: "+urlService);
		return client.target(urlService).request(MediaType.APPLICATION_JSON)
				     .post(Entity.entity(accionDTO, MediaType.APPLICATION_JSON), AccionDTO.class);
	}        
	
	public AccionDTO editarAccion(AccionDTO accionDTO) throws Exception {
		Client client = ClientBuilder.newClient();
		System.out.println("Ejecutando: "+urlService);
		return client.target(urlService).request(MediaType.APPLICATION_JSON)
				     .put(Entity.entity(accionDTO, MediaType.APPLICATION_JSON), AccionDTO.class);
	}
	
	public List<AccionDTO> getAcciones() throws Exception {
		try {
			Client client = ClientBuilder.newClient();
			System.out.println("Ejecutando: "+urlService);
			WebTarget messages = client.target(urlService);
			return messages.request(MediaType.APPLICATION_JSON_TYPE)
					       .get(new GenericType<List<AccionDTO>>() {} );
		} catch (Exception ex) {
