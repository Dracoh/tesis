...
public class AplicacionWR {
	...
	private static final String PATH_PROVEEDOR = "aplicacion"; 
	...
	public AplicacionWR(String method) {
	...
	public AplicacionDTO crearAplicacion(AplicacionDTO aplicacionDTO) throws Exception {
		Client client = ClientBuilder.newClient();
		System.out.println("Ejecutando: "+urlService);
		return client.target(urlService).request(MediaType.APPLICATION_JSON)
				     .post(Entity.entity(aplicacionDTO, MediaType.APPLICATION_JSON), AplicacionDTO.class);
	}        
	
	public AplicacionDTO editarAplicacion(AplicacionDTO aplicacionDTO) throws Exception {
		Client client = ClientBuilder.newClient();
		System.out.println("Ejecutando: "+urlService);
		return client.target(urlService).request(MediaType.APPLICATION_JSON)
				     .put(Entity.entity(aplicacionDTO, MediaType.APPLICATION_JSON), AplicacionDTO.class);
	}
	
	public List<AplicacionDTO> getAplicaciones() throws Exception {
		try {
			Client client = ClientBuilder.newClient();
			System.out.println("Ejecutando: "+urlService);
			WebTarget messages = client.target(urlService);
			return messages.request(MediaType.APPLICATION_JSON_TYPE)
					       .get(new GenericType<List<AplicacionDTO>>() {} );
		} catch (Exception ex) {
