...
public class UsuarioDTO implements Serializable {
	...
	public UsuarioDTO(Usuario usuario) {
		...
		this.usuDireccion = usuario.getUsuDireccion();
		this.usuFechaNacimiento = usuario.getUsuFechaNacimiento();
		this.usuEstado = usuario.isUsuEstado();

		/* Llaves Foraneas */
		// UserMembership
		if (usuario.getUserMembership() != null && usuario.getUserMembership().getUmpCodigo() > 0L) {
			userMembershipDTO = new UserMembershipDTO();
			userMembershipDTO.setUmpCodigo(usuario.getUserMembership().getUmpCodigo());
		}
